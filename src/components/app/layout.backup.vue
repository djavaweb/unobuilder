<template>
	...
	<div class="right-panel" @mouseleave="clearCanvas()">
		...
		<div class="right-panel-container" v-show="isRightBarView('settings')">
...
export default {
	...
	methods: {
		 ...
		 */
		copy () {
			this.layout.$emit('copyElement', this.outline.selected.id)
		 ...
		 */
		remove () {
			this.layout.$emit('removeElement', this.outline.selected.id)
			this.$emit('disableEditor')
		},
		...
		addBlock (block) {
			if (this.layout) {
				this.layout.$emit('addBlock', block)
			}
		},
	...
	events: {
		...
		viewerReady () {
			let self = this,
			layout = self.getCanvasIframe('body').__vue__
