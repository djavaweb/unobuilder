...
const App = new Vue({
	...
	methods: {
		createElement (element, watch) {
			...
			el.$watch = (old, val) => {
				watch && watch.call(this, old, val);
			}

			el.$set =function (key, val, screen) {
				var oldval = this.$get(key, screen);
				this.$get[key] = val;
				el.$watch.call(el, oldval, val);
			}
			...
			el.$get = (key, screen) => {
				return this.props[this.$screen(screen)]
			}
